...
const STORAGE_KEY = "plomberiequest_state_v1";
...
const ACCESS_CODE = "PLOMBERIE-2026-ERD9"; // code fourni au centre de formation après achat de la licence
...
const SYNC_KEY = "plomberiequest_sync_pending";
...
function deviceId() {
  let id = localStorage.getItem("plomberiequest_device_id");
  if (!id) {
    id = "cq-" + (crypto.randomUUID ? crypto.randomUUID() : Date.now().toString(36) + Math.random().toString(36).slice(2));
    localStorage.setItem("plomberiequest_device_id", id);
  }
  return id;
}
